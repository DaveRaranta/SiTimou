...
class AuthServices {
  ...
  static Future<bool> masuk(String userNik, String userPwd) async {
    final url = Uri.parse('$mainUrl/auth/masuk/');
  ...
  static Future<bool> saveRegistrasiPengguna(
    String userNik,
    String userName,
    String userPwd,
  ) async {
    final url = Uri.parse('$mainUrl/auth/daftar');
    ...
    try {
      var result = await client.post(url, body: payload).timeout(const Duration(seconds: 30));

      debugPrint("[!] DebugInfo: [INFO] AuthServices.saveRegistrasiPengguna() => ${result.body.toString()}");
      ...
      debugPrint("[!] DebugInfo: [ERROR] AuthServices.saveRegistrasiPegawai() => ${e.toString()}");
  ...
  static Future<bool> validasiPengguna() async {
    final url = Uri.parse('$mainUrl/auth/status_pengguna/${g.userId}');
  ...
  static Future<bool> updateFcmToken(String? value) async {
    final url = Uri.parse('$mainUrl/auth/update_fcm_token');
